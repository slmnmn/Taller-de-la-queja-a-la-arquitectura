# Taller: De la Queja a la Arquitectura
## Simón Alejandro Rincón Rojas

Tras aplicar las encuestas, podemos darnos cuenta de que hay 2 problemas recurrentes.

![Resultados pregunta 1.](/imagenes/p1.png "Pregunta 1")

### 1. El problema (Discovery)
#### ¿Qué duele?
> "El fin de semana pasado revisé mi cuenta y me di cuenta de que me había gastado casi todo el dinero que tenia para la quincena, pero no supe en que"

Sin duda alguna, tambien una de las respuestas que nos demuestra un impacto real y super importante que termina teninendo repercusiones en nuestro o su diario vivir.

#### Frecuencia 
![Resultados pregunta 3.](/imagenes/p3.png "Pregunta 3")

Teniendo en cuenta todas las respuestas de todos los participantes en la pregunta 3 el promedio de la misma es 3.11.

Sin embargo, teniendo en cuenta solo las respuestas de los participantes que repondieron la pregunta #1 con el problema de la gestión del dinero, nos da un promedio de 3.75 (Medianamente recurrente)

#### La solución soñada

> Una app q cada vez que compre algo me recolecte toda la informacion de mis compras con precios, detalles y depronto q me de consejos de como mejorar mis finanzas o que tambien me enseñe en que deberia gastar mejor el dinero.

>Que registre automáticamente mis compras al momento de realizarlas, ya sea con tarjeta o en efectivo, y que me muestre al instante cuánto dinero me queda para el resto del mes sin tener que hacer cálculos yo mismo

> Una app que me ayude a administrar mi dinero, en la que yo pueda indicar cuáles han sido mis gastos y que me diga no solo la información sino en lo posible, un plan de ahorro con IA

> Separar la cantidad de dinero que va a X rubro de las cosas que tenga por pagar mensualmente

### 2 Definición funcional
#### Historia de usuario principal

Para la definición funcional, la __actividad__ teniendo las diferentes "soluciones deseadas"

![Actividad.](/imagenes/actividad.png "Actividad")

Esta actividad engloba la necesidad de saber qué entra, qué sale y cuánto queda de forma centralizada.

El __paso a paso__ que podemos sacar de las respuestas puede ser el siguiente.

![Paso a paso](/imagenes/image.png "paso a paso")

Ya sacando la __historia de usuario__ con el paso a paso queda de esta manera:

![historia](/imagenes/historia.png "historia")

### Criterios de aceptación 

- No debe demorarse mas de 5 minutos en actualizar los datos.
- Las compras hechas tienen que estar categorizadas.
- Actualizar el presupuesto o monto disponible para la quincena.
- Tiene que ser a tiempo real. Actualizandose con cada compra.
- Accesible desde varios medios.


#### Requisitos funcionales
- El sistema debe integrarse mediante APIs bancarias seguras (Open Banking) para capturar transacciones en tiempo real y clasificarlas automáticamente en categorías de gasto. (Automatiza)

- El sistema debe calcular dinámicamente un "Presupuesto Diario de Supervivencia", restando los gastos fijos mensuales y metas de ahorro del saldo total, mostrando al usuario cuánto puede gastar hoy sin comprometer el dinero de la siguiente quincena. (Proyecta)

- El sistema debe generar un informe de diagnóstico semanal mediante alguna inteligencia artificial que identifique "gastos hormiga" o patrones de consumo ineficientes, proponiendo automáticamente un ajuste en los límites de gasto de la categoría afectada para el siguiente periodo.